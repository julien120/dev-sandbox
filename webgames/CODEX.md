...
- julien: Webカメラで映る人物がジュリアン本人かどうかを推定する認証プロトタイプ。
  - 当面はダミー推定（輝度ベース）で動作させ、後日渡される埋め込み＋モデルへ差し替え可能な構造にする。
  - スコア（0-100）とメッセージを画面に表示し、UI/UX を整備する。
- apoc: 真っ黒なキャンバスに白のラインで骨格を描く A-POC 風モーションビジュアライザ。
  - MediaPipe Pose を使って骨格座標を取得し、Canvas で白いワイヤーと残像を描く。
  - カメラ入力は保存せず、描画のみ行う。
