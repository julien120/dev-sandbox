import { Game, Scene, clamp, lerp, vec2, } from '@webgames/engine';
import './style.css';
const FRUITS = [
...
class SuikaScene extends Scene {
    constructor() {
        ...
        Object.defineProperty(this, "nextFruitId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 1
        });
        Object.defineProperty(this, "activeFruitId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "previewType", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "spawnBag", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "dropCooldown", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "pointerX", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "pendingMerges", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "canvasRect", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
    ...
    update(dt, context) {
        ...
        if (activeFruit) {
            ...
            if (this.pointerX !== null) {
                const target = clamp(this.pointerX, PLAYFIELD_LEFT + activeFruit.radius, PLAYFIELD_RIGHT - activeFruit.radius);
                activeFruit.position.x = clamp(lerp(activeFruit.position.x, target, 0.35), PLAYFIELD_LEFT + activeFruit.radius, PLAYFIELD_RIGHT - activeFruit.radius);
            }
            activeFruit.position.x = clamp(activeFruit.position.x, PLAYFIELD_LEFT + activeFruit.radius, PLAYFIELD_RIGHT - activeFruit.radius);
    ...
    render(context) {
        ...
        ctx.fillRect(PLAYFIELD_LEFT - WALL_THICKNESS, PLAYFIELD_TOP - WALL_THICKNESS, PLAYFIELD_WIDTH + WALL_THICKNESS * 2, PLAYFIELD_HEIGHT + WALL_THICKNESS * 2);
        ...
        this.fruits.forEach((fruit) => {
            ...
            const gradient = ctx.createRadialGradient(-fruit.radius * 0.3, -fruit.radius * 0.3, fruit.radius * 0.4, 0, 0, fruit.radius);
    ...
    updatePointerPosition(event) {
        const canvas = event.currentTarget ?? document.getElementById('game-canvas');
        if (!canvas) {
            return;
        }
        this.canvasRect = canvas.getBoundingClientRect();
    ...
    spawnControlledFruit(type) {
        const radius = FRUITS[type].radius;
        const startX = clamp(this.pointerX ?? PLAYFIELD_LEFT + PLAYFIELD_WIDTH / 2, PLAYFIELD_LEFT + radius, PLAYFIELD_RIGHT - radius);
        const fruit = {
            id: this.nextFruitId++,
            type,
            radius,
    ...
    maybeQueueMerge(a, b, distance, minDistance) {
        if (this.gameOver ||
            a.type !== b.type ||
    ...
    checkGameOver() {
        ...
        const overflow = this.fruits.some((fruit) => !fruit.isControlled &&
            fruit.age > 1 &&
